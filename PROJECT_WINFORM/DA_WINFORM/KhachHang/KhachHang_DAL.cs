...
namespace DA_WINFORM
{
    class KhachHang_DAL
    {
        ...
        public string GenerateNewMaKH()
        {
            ...
            if (result != null)
            {
                string lastMaKH = result.ToString().Substring(2); // Lấy phần số của MAKH
                int newMaKH = int.Parse(lastMaKH) + 1;
                return "KH" + newMaKH.ToString("D2"); // Đảm bảo luôn có 2 chữ số
            }
            return "KH01"; // Nếu chưa có khách hàng nào
        }
